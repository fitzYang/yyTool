...
public class CookieDataCodec {
    ...
    public static void decode(Map<String, String> map, String data) throws UnsupportedEncodingException {
        String[] keyValues = data.split(StringPool.AMPERSAND);
        for (String keyValue : keyValues) {
            String[] splitted = keyValue.split(StringPool.EQUALS, 2);
            if (splitted.length == 2) {
                final String decodeKey = URLDecoder.decode(splitted[0], StringPool.UTF_8);
                final String decodeVal = URLDecoder.decode(splitted[1], StringPool.UTF_8);
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            if (entry.getValue() != null) {
                data.append(separator)
                        .append(URLEncoder.encode(entry.getKey(), StringPool.UTF_8))
                        .append(StringPool.EQUALS)
                        .append(URLEncoder.encode(entry.getValue(), StringPool.UTF_8));
                separator = StringPool.AMPERSAND;
            }
        }
